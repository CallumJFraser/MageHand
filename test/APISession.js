...
describe('GetSession -', function(){
	...
	describe('Valid call:', function(){
		it('Should return Success:true', function(done){
			dndAPI.Login(validUsername, validPassword, function(loginResponse){
				dndAPI.GetSession(loginResponse.AID, loginResponse.SessionID, 1, function(result){
	...
	describe('Invalid call (missing param: "RunthroughID"):', function(){
		it('Should return a Result', function(done){
			dndAPI.Login(validUsername, validPassword, function(loginResponse){
				dndAPI.GetSession(loginResponse.AID, loginResponse.SessionID, undefined, function(result){
					assert.notEqual(result.Result, undefined);
					done();
				});
			});
