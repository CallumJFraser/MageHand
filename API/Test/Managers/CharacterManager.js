...
describe('Character Manager', function() {
	...
	describe('GetByAccount:', function(){
		...
		it('Valid:', function(done){
			...
			fakeManager.GetByAccount(valid).then(function(characters){
				var character = characters[0];
				assert.notEqual(character, undefined);
				assert.equal(character.Reason, undefined);
				assert.equal(character.ID, validObject.ID);
				assert.equal(character.Name, validObject.Name);
				assert.notEqual(character.Class, undefined);
				assert.equal(character.Experiance, validObject.Experiance);
				assert.notEqual(character.Race, undefined);
				assert.equal(character.Age, validObject.Age);
				assert.equal(character.Height, validObject.Height);
				assert.equal(character.Strength, validObject.Strength);
				assert.equal(character.Dexterity, validObject.Dexterity);
				assert.equal(character.Constitution, validObject.Constitution);
				assert.equal(character.Inteligence, validObject.Inteligence);
				assert.equal(character.Wisdom, validObject.Wisdom);
				assert.equal(character.Charisma, validObject.Charisma);
				assert.equal(character.HP, validObject.HP);
				assert.equal(character.AC, validObject.AC);
				assert.equal(character.Fortitude, validObject.Fortitude);
				assert.equal(character.Reflex, validObject.Reflex);
				assert.equal(character.Will, validObject.Will);
				assert.equal(character.Grapple, validObject.Grapple);
				assert.equal(character.BaseAttack, validObject.BaseAttack);
				assert.equal(character.SpellResistance, validObject.SpellResistance);
				assert.equal(character.TouchAC, validObject.TouchAC);
				assert.equal(character.FlatFootedAC, validObject.FlatFootedAC);
				done();
			});
		})
